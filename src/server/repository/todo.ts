//import { read, update, deleteById as dbDeleteById } from "@db-crud-todo";
...
async function get({
  ...
  const parseData = TodoSchema.array().safeParse(data);

  if (!parseData.success) throw new Error("Failed to parse TODO from database");
