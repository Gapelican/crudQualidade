...
function get({
  ...
  return fetch(`/api/todos?page=${page}&limit=${limit}`).then(
    async (respostaDoServidor) => {
      const todosSpring = await respostaDoServidor.text();
      // como garantir a tipagem de tipos desconhecidos?
      const responseParsed = parseTodosFromServer(JSON.parse(todosSpring));
...
async function toggleDone(todoId: string): Promise<Todo> {
  const response = await fetch(
    ...
    `https://crud-phi-sepia.vercel.app/api/todos/${todoId}/toggle-done`,
    { method: "PUT" }
  );
